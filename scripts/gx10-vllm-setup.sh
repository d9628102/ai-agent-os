...
# invisible character picked up from a copy-paste, e.g. a full-width space
# or smart quote) doesn't fail here — it fails deep inside vLLM/httpx when
# it tries to encode the "Authorization: Bearer <token>" header, crashing
# the container in a loop with a cryptic UnicodeEncodeError. See
# docs/gx10-known-issues.md #7.
